  ...
  function homePage(project) {
    ...
    function animationRow(data) {
      ...
      let row = `<td class="has-text-centered">${data.viewed}</td>` +
        `<td><a class="has-text-light" href="${url}" title="${data.description}">${title}</a></td>`;
      if (data.duration !== 0)
    ...
    function simulationRow(data) {
      ...
      const row =
        `<td class="has-text-centered"><a class="has-text-light" href="${repository}/stargazers" target="_blank" title="GitHub stars">` +
        `${data.stars}</a></td>` +
        `<td><a class="has-text-light" href="/run?url=${data.url}" title="${data.description}">${title}</a></td>` +
        `<td><a class="has-text-light" href="${data.url}" target="_blank" title="View GitHub repository">${words[3]}</a></td>` +
    ...
    function serverRow(data) {
      ...
      const row =
        `<td><a class="has-text-light" href="${data.url}/monitor" target="_blank">${name}</a></td>` +
      ...
      fetch('ajax/project/create.php', {method: 'post', body: JSON.stringify({url: url, id: id})})
        ...
        .then(function(data) {
          ...
          if (data.error) {
            ...
            if (errorMsg !== "Failed to update the simulation")
