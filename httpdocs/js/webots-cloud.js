...
document.addEventListener('DOMContentLoaded', function() {
  ...
  function runPage(project) {
    ...
    function getBenchmark(url) {
      ...
      fetch(rawUrl + '/README.md?nocache=' + (new Date()).getTime(), {cache: 'no-cache'})
      ...
      fetch(rawUrl + '/webots.yml?nocache=' + (new Date()).getTime(), {cache: 'no-cache'})
        .then(function(response) { return response.text(); })
        .then(function(data) {
          metric = data.match(/metric: ([a-z-]+)/)[1];
        });

      fetch(rawUrl + '/competitors.txt?nocache=' + (new Date()).getTime(), {cache: 'no-cache'})
        .then(function(response) { return response.text(); })
        .then(function(data) {
