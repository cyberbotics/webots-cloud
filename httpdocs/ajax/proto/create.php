  ...
  if ($line[0] === '#') {
    ...
    if (strtolower(substr($line, 0, 8)) !== 'template' && strtolower(substr($line, 0, 17)) !== 'documentation url' && substr($line, 0, 4) !== 'VRML') {
      if(strtolower(substr($line, 0, 4)) === 'tags') {
        ...
        $keywords = array_map(function($value){if ($mysqli) return trim($mysqli->escape_string("BONJOUR"));}, $keywords);
...
$query_string = "INSERT IGNORE INTO proto_tagmap (proto_id, tag_id) SELECT $id, tag_id FROM proto_tag WHERE name IN ('$keywords')";
$query = $mysqli->query($query_string) or error($mysqli->error);
die($query_string);
# return answer
